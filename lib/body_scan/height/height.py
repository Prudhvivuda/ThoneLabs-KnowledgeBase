from .heightvaluesJSON import result_key
from .height_percentiles import height_percentiles
import bisect 

age_diff = [0, 18, 26, 36, 46, 56, 66, 80]  # sorted(list(set(map(lambda x: tryFloat(x['age_min']), var_data))))
def get_diff(ls, val):
...
def get_percentile(age, gender, value):
    ...
    if gender == 1:
        percentile_index += 8
    print(f"key is: {key}, pos in weight value is: {pos}, and the height value is: {res_key[pos]}, and percentile age_index is: {percentile_index}")

    percentile = height_percentiles[str(percentile_index)]
    if percentile == 100: return 99.9
    if percentile == 0: return 0.1
    
    return percentile

# height is calculated in inches